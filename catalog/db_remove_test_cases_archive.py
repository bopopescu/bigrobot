...
def find_test_data():
    cat = TestCatalog()
    test_count = cat.find_test_cases_archive_matching_build(
                        os.environ['BUILD_NAME']).count()
    print("Found tests(%s) matching build '%s'."
          % (test_count, os.environ['BUILD_NAME']))
    return (test_count)

def remove_test_data():
    cat = TestCatalog()
    test_count = cat.remove_test_cases_archive_matching_build(
                        os.environ['BUILD_NAME'])
    print("Removed tests(%s) matching build '%s'."
          % (test_count, os.environ['BUILD_NAME']))
    return (test_count)
