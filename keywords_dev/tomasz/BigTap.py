...
######### To Be Added #########
    def rest_add_tacacs_server(self):
        '''
            Objective:
            - Execute the CLI command 'tacacs server host 10.192.4.91 key secret'
        ...
        try:
            ...
            c= t.controller()
            try:
                url = '/api/v1/data/controller/core/aaa/tacacs/server[server-address="10.192.4.91"]'
                data =  {"server-address": "10.192.4.91", "secret": "71172f28362d27"}
                c.rest.put(url, data)
            except:
    ...
    def rest_aaa_authz_config_local_remote(self):
        '''
            Objective:
            - Execute the CLI command 'aaa authorization exec default local group tacacs+'
        ...
        try:
            ...
            try:
                ...
                data = [{"priority": 1, "name": "local"}, {"priority": 2, "name": "tacacs"}]
                c.rest.put(url, data)
            except:
                helpers.test_failure(c.rest.error())
                return False
    ...
    def rest_aaa_authn_config_local_remote(self):
        '''
            Objective:
            - Execute the CLI command 'aaa authentication login default local group tacacs+'
