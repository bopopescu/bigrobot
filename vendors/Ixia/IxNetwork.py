...
class IxNet:
	def __init__(self):
		...
		self._sendContent = str()
		self._buffer = False
		self._sendBuffer = str()
		self._decoratedResult = str()
		self._filename = None
		self._debug = False
		self._async = False
		self._timeout = None
		self._OK = '::ixNet::OK'
		self._version = '7.0.801.25'
	...
	def __SendRecv(self, *args):
		...
		self._sendBuffer += self._sendContent+'\03'
	#	print("Sending: ",self._sendBuffer)
		if self._buffer == False:
			if self._debug:
				print("Sending: ",self._sendBuffer)
			self.__Send("<001><002><009{0}>{1}".format(len(self._sendBuffer),
				self._sendBuffer))
			self._sendBuffer = str()
	...
	def __Recv(self):
		...
		try:
			while True:
				...
				while True:
					responseBuffer += self._socket.recv(1)
				...
				if commandId == 1:
					...
					self._evalResult = self._socket.recv(contentLength)
				elif commandId == 7:
					self._filename = self._socket.recv(contentLength)
				elif commandId == 8:
		...
		if self._debug:
			print("Received: ",self._decoratedResult)

		if self._evalResult == self._evalError:
			raise IxNetError(self._decoratedResult)

		if self._decoratedResult.startswith('['):
			return eval(self._decoratedResult)
		else:
