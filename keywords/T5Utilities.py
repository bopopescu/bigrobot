...
class T5Utilities(object):
    ...
    def start_floodlight_monitor(self, monitor_message="Exception"):
    ...
    def restart_floodlight_monitor(self, node, monitor_message='Exception'):
    ...
    def cli_run(self, node, command, cmd_timeout=60, user='admin',
        ...
        if (node == 'main') or (node == 'subordinate'):
...
class T5PlatformThreads(Thread):
    ...
    def run(self):
        ...
        if(self.name == "activeReboot"):
            self.controller_reboot('main')
        if(self.name == "standbyReboot"):
            self.controller_reboot('subordinate')
