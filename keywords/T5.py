...
class T5(object):
    ...
    def rest_add_vns(self, tenant, vns):
        ...
        url = '/api/v1/data/controller/applications/bvs/tenant[name="%s"]/segment[name="%s"]' % (tenant, vns)
        try:
    ...
    def rest_add_endpoint(self, tenant, vns, endpoint):
        ...
        url = '/api/v1/data/controller/applications/bvs/tenant[name="%s"]/segment[name="%s"]/endpoint' % (tenant, vns)
        try:
    ...
    def rest_delete_endpoint(self, tenant, vns, endpoint=None):
        ...
        url = '/api/v1/data/controller/applications/bvs/tenant[name="%s"]/segment[name="%s"]/endpoint[name="%s"]' % (tenant, vns, endpoint)
        try:
    ...
    def rest_add_interface_to_portgroup(self, switch, intf, pg):
        ...
        url = '/api/v1/data/controller/applications/bvs/port-group[name="%s"]/member-interface[switch-name="%s"][interface-name="%s"]' % (pg, switch, intf)
        try:
    ...
    def rest_add_portgroup_to_vns(self, tenant, vns, pg, vlan):
        ...
        url = '/api/v1/data/controller/applications/bvs/tenant[name="%s"]/segment[name="%s"]/port-group-membership-rule[port-group="%s"]' % (tenant, vns, pg)
    ...
    def rest_add_portgroup_to_endpoint(self, tenant, vns, endpoint, pg, vlan):
        ...
        url = '/api/v1/data/controller/applications/bvs/tenant[name="%s"]/segment[name="%s"]/endpoint[name="%s"]/attachment-point' % (tenant, vns, endpoint)
        try:
    ...
    def rest_add_interface_to_vns(self, tenant, vns, switch, intf, vlan):
        ...
        url = '/api/v1/data/controller/applications/bvs/tenant[name="%s"]/segment[name="%s"]/switch-port-membership-rule[switch="%s"][interface="%s"]' % (tenant, vns, switch, intf)
        try:
    ...
    def rest_verify_forwarding_vlan_table(self, switch):
        ...
        t = test.Test()
        c = t.controller('master')
        url = '/api/v1/data/controller/applications/bvs/info/forwarding/network/switch[switch-name="%s"]?select=vlan-table' % (switch)
        c.rest.get(url)
        data = c.rest.content()
        no_of_vlans = len(data[0]["vlan-table"])
        no_of_user_vlan = int(no_of_vlans) - 1
        url1 = '/api/v1/data/controller/applications/bvs/info/endpoint-manager/segment' % ()
        c.rest.get(url1)
    ...
    def rest_verify_forwarding_port(self, switch):
        ...
        t = test.Test()
        c = t.controller('master')
        url = '/api/v1/data/controller/applications/bvs/info/forwarding/network/switch[switch-name="%s"]?select=port-table' % (switch)
        c.rest.get(url)
        data = c.rest.content()
        for i in range(0, len(data[0]["port-table"])):
            if ((data[0]["port-table"][i]["lag-id"]) == 0):
    ...
    def rest_verify_forwarding_vlan_xlate(self, switch, vlan, intf):
        ...
        lag_id = []
        for i in range(0, len(data[0]["port-table"])):
            if str(data[0]["port-table"][i]["port-num"]) == str(interface):
    ...
    def rest_verify_forwarding_vlan_fabric_tag_members(self, switch):
        ...
        url1 = '/api/v1/data/controller/applications/bvs/info/forwarding/network/switch[switch-name="%s"]?select=vlan-member-table' % (switch)
        c.rest.get(url1)
        data1 = c.rest.content()
        for i in range(0, len(data1[0]["vlan-member-table"])):
                list_common = list(set(list_fabric_interface).intersection(set(data1[0]["vlan-member-table"][i]["tagged-port"])))
    ...
    def rest_verify_forwarding_vlan_edge_untag_members(self, switch, intf):
        ...
        t = test.Test()
        c = t.controller('master')
        url = '/api/v1/data/controller/applications/bvs/info/forwarding/network/switch[switch-name="%s"]?select=vlan-member-table' % (switch)
        c.rest.get(url)
        data = c.rest.content()
        interface = re.sub("\D", "", intf)
        for i in range(0, len(data[0]["vlan-member-table"])):
            try:
                value = data[0]["vlan-member-table"][i]["untagged-port"]
            except KeyError:
                continue
            if str(interface) in data[0]["vlan-member-table"][i]["untagged-port"]:
    ...
    def rest_verify_forwarding_vlan_edge_tag_members(self, switch, intf):
        ...
        for i in range(0, len(data[0]["vlan-member-table"])):
            try:
                value = data[0]["vlan-member-table"][i]["tagged-port"]
            except KeyError:
                continue
            if interface in data[0]["vlan-member-table"][i]["tagged-port"]:
    ...
    def rest_verify_forwarding_layer2_table_untag(self, switch, intf, mac):
        ...
        url3 = '/api/v1/data/controller/applications/bvs/info/forwarding/network/switch[switch-name="%s"]?select=l2-table' % (switch)
        c.rest.get(url3)
    ...
    def rest_add_endpoint_scale(self, tenant, vns, mac, endpoint, switch, intf, vlan, count):
        ...
        while (i <= int(count)):
            ...
            url1 = '/api/v1/data/controller/applications/bvs/tenant[name="%s"]/segment[name="%s"]/endpoint[name="%s"]/attachment-point' % (tenant, vns, endpoint_new)
            c.rest.put(url1, {"switch": switch, "interface": intf, "vlan": vlan})
            url2 = '/api/v1/data/controller/applications/bvs/tenant[name="%s"]/segment[name="%s"]/endpoint[name="%s"]' % (tenant, vns, endpoint_new)
    ...
    def rest_verify_vns_rx_stats(self, tenant, vns, frame_cnt, vrange=5):
        ...
        vrange = int(vrange)
        url = '/api/v1/data/controller/applications/bvs/info/stats/segment/stats[name="%s"][tenant-name="%s"]' % (vns, tenant)
        c.rest.get(url)
        data = c.rest.content()
        if data[0]["tenant-name"] == tenant and data[0]["name"] == vns:
                    if (int(data[0]["rx-packet"]) >= (frame_cnt - vrange)) and (int(data[0]["rx-packet"]) <= (frame_cnt + vrange)):
                        helpers.log("Pass: Counters value Expected:%d, Actual:%d" % (frame_cnt, int(data[0]["rx-packet"])))
                        return True
                    else:
                        helpers.test_failure("Vns counter value does not match,Expected:%d,Actual:%d" % (frame_cnt, int(data[0]["rx-packet"])))
                        return False
        else:
    ...
    def rest_verify_vns_rx_rates(self, tenant, vns, frame_rate, vrange=5):
        ...
        data = c.rest.content()
        if data[0]["tenant-name"] == tenant and data[0]["name"] == vns:
            if (int(data[0]["rx-packet-rate"]) >= (frame_rate - vrange)) and (int(data[0]["rx-packet-rate"]) <= (frame_rate + vrange)):
                helpers.log("Pass: Rate value Expected:%d, Actual:%d" % (frame_rate, int(data[0]["rx-packet-rate"])))
                return True
            else:
                helpers.test_failure("Vns rate value does not match,Expected:%d,Actual:%d" % (frame_rate, int(data[0]["rx-packet-rate"])))
                return False
        else:
    ...
    def rest_verify_vns_tx_stats(self, tenant, vns, frame_cnt, vrange=5):
        ...
        data = c.rest.content()
        if data[0]["tenant-name"] == tenant and data[0]["name"] == vns:
                    if (int(data[0]["tx-packet"]) >= (frame_cnt - vrange)) and (int(data[0]["tx-packet"]) <= (frame_cnt + vrange)):
                        helpers.log("Pass: Counters value Expected:%d, Actual:%d" % (frame_cnt, int(data[0]["tx-packet"])))
                        return True
                    else:
                        helpers.test_failure("vns counters does not match, Expected:%d,Actual:%d" % (frame_cnt, int(data[0]["tx-packet"])))
                        return False
        else:
            helpers.log("Given tenant name and VNS name does not match the config")
    ...
    def rest_verify_vns_tx_rates(self, tenant, vns, frame_rate, vrange=5):
        ...
        data = c.rest.content()
        if data[0]["tenant-name"] == tenant and data[0]["name"] == vns:
            if (int(data[0]["tx-packet-rate"]) >= (frame_rate - vrange)) and (int(data[0]["tx-packet-rate"]) <= (frame_rate + vrange)):
                helpers.log("Pass: Rate value Expected:%d, Actual:%d" % (frame_rate, int(data[0]["tx-packet-rate"])))
                return True
            else:
                helpers.test_failure("vns rates does not match, Expected:%d,Actual:%d" % (frame_rate, int(data[0]["tx-packet-rate"])))
                return False
        else:
            helpers.log("Given tenant name and vns name does not match in the config")
    ...
    def rest_clear_fabric_interface_stats(self):
        ...
        t = test.Test()
        c = t.controller('master')
        url = '/api/v1/data/controller/applications/bvs/info/stats/interface/stats'
        c.rest.delete(url, {})
        ...
        if len(peer_intf) != 0:
            if data1[0]["leaf-group"] == None:
                for i in range(0, len(data[0]["lag-table"])):
                        if peer_intf[0] in data[0]["lag-table"][i]["port"]:
    ...
    def rest_verify_forwarding_port_edge(self, switcha, intf0, switchb, intf1):
        ...
        url_a = '/api/v1/data/controller/applications/bvs/info/forwarding/network/switch[switch-name="%s"]?select=port-table' % (switcha)
        c.rest.get(url_a)
        data = c.rest.content()
        url_b = '/api/v1/data/controller/applications/bvs/info/forwarding/network/switch[switch-name="%s"]?select=port-table' % (switchb)
        c.rest.get(url_b)
        data1 = c.rest.content()
        lag_id_a = []
        lag_id_b = []
        for i in range(0, len(data[0]["port-table"])):
            if str(data[0]["port-table"][i]["port-num"]) == str(intf0):
                    helpers.log("adding the lag id=%s" % data[0]["port-table"][i]["lag-id"])
                    lag_id_a.append(data[0]["port-table"][i]["lag-id"])
            else:
                continue
        for i in range(0, len(data1[0]["port-table"])):
            if str(data1[0]["port-table"][i]["port-num"]) == str(intf1):
                lag_id_b.append(data1[0]["port-table"][i]["lag-id"])
            else:
                continue
    ...
    def rest_verify_forwarding_port_source_mac_check(self, switch):
        ...
        for i in range(0, len(data)):
            if data[i]["type"] == "unknown":
                ...
                list_fabric_interface.append(int(re.sub("\D", "", (data[i]["name"]))))
        url1 = '/api/v1/data/controller/applications/bvs/info/forwarding/network/switch[switch-name="%s"]?select=port-table' % (switch)
        c.rest.get(url1)
        data1 = c.rest.content()
        if len(list_fabric_interface) != 0 or len(list_fabric_edge_interface) != 0:
            for i in range(0, len(data1[0]["port-table"])):
                if data1[0]["port-table"][i]["port-num"] in list_fabric_interface:
                    if data1[0]["port-table"][i]["is-src-mac-check-disabled"] == True:
                        helpers.log("Source Mac check is disabled on Fabric interfaces")
                        return True
                    else:
                        helpers.test_failure("Source MAC check is enabled on Fabric Interface:%s" % data1[0]["port-table"][i]["port-num"])
                        return False
                elif data1[0]["port-table"][i]["port-num"] in list_fabric_edge_interface:
                    if data1[0]["port-table"][i]["is-src-mac-check-disabled"] == False:
                        helpers.log("Source MAC check is enabled on fabric edge interfaces")
                        return True
                    else:
                        helpers.log("Source MAC check is disabled on fabric edge interface:%s" % data1[0]["port-table"][i]["port-num"])
                        return False
        return False
    ...
    def rest_verify_tenant_rx_stats(self, tenant, frame_cnt, vrange=5):
        ...
        frame_cnt = int(frame_cnt)
        vrange = int(vrange)
        url = '/api/v1/data/controller/applications/bvs/info/stats/tenant/stats[name="%s"]' % (tenant)
        c.rest.get(url)
        data = c.rest.content()
        if data[0]["name"] == tenant:
                    if (int(data[0]["rx-packet"]) >= (frame_cnt - vrange)) and (int(data[0]["rx-packet"]) <= (frame_cnt + vrange)):
                        helpers.log("Pass: Tenant Counters value Expected:%d, Actual:%d" % (frame_cnt, int(data[0]["rx-packet"])))
                        return True
                    else:
                        helpers.test_failure("Tenant counter value does not match,Expected:%d,Actual:%d" % (frame_cnt, int(data[0]["rx-packet"])))
                        return False
        else:
    ...
    def rest_verify_tenant_tx_stats(self, tenant, frame_cnt, vrange=5):
        ...
        if data[0]["name"] == tenant:
                    if (int(data[0]["tx-packet"]) >= (frame_cnt - vrange)) and (int(data[0]["tx-packet"]) <= (frame_cnt + vrange)):
                        helpers.log("Pass: Tenant Counters value Expected:%d, Actual:%d" % (frame_cnt, int(data[0]["tx-packet"])))
                        return True
                    else:
                        helpers.test_failure("Tenant counter value does not match,Expected:%d,Actual:%d" % (frame_cnt, int(data[0]["tx-packet"])))
                        return False
        else:
            helpers.log("Given tenant name does not match the config")
    ...
    def rest_verify_tenant_rx_rates(self, tenant, frame_rate, vrange=5):
        ...
        if data[0]["name"] == tenant:
                    if (int(data[0]["rx-packet-rate"]) >= (frame_rate - vrange)) and (int(data[0]["rx-packet-rate"]) <= (frame_rate + vrange)):
                        helpers.log("Pass: Tenant Counters value Expected:%d, Actual:%d" % (frame_rate, int(data[0]["rx-packet-rate"])))
                        return True
                    else:
                        helpers.test_failure("Tenant counter value does not match,Expected:%d,Actual:%d" % (frame_rate, int(data[0]["rx-packet-rate"])))
                        return False
        else:
            helpers.log("Given tenant name does not match the config")
    ...
    def rest_verify_tenant_tx_rates(self, tenant, frame_rate, vrange=5):
        ...
        if data[0]["name"] == tenant:
                    if (int(data[0]["tx-packet-rate"]) >= (frame_rate - vrange)) and (int(data[0]["tx-packet-rate"]) <= (frame_rate + vrange)):
                        helpers.log("Pass: Tenant Counters value Expected:%d, Actual:%d" % (frame_rate, int(data[0]["tx-packet-rate"])))
                        return True
                    else:
                        helpers.test_failure("Tenant counter value does not match,Expected:%d,Actual:%d" % (frame_rate, int(data[0]["tx-packet-rate"])))
                        return False
        else:
            helpers.log("Given tenant name does not match the config")
