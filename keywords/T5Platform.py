...
#        n_console.expect(r'Please choose an option:.*[\r\n$]')
    ...
    def cli_walk_exec(self, string='', file_name=None, padding=''):
        ...
        for line in temp:
            ...
            if key == "ApplicationManager" or key == "Controller" or key == "EndpointManager" or key == "OFSwitchManager" or \
            key == "FabricManager" or key == "com.bigswitch.floodlight.bvs.application" or key == "ForwardingDebugCounters" or \
            key == "ISyncService" or key == "StatsCollector" or key == "VirtualRoutingManager" or key == "org.projectfloodlight.core" or \
            key == "StatsCollector":
                helpers.log("Ignore line %s" % line)
                num = num - 1
                continue
